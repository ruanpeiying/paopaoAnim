...
/**
 * Created by ruanpeiying on 16/4/14.
 */
...
public class PeriscopeLayout extends RelativeLayout {
        ...
        private void init() {
            ...
            Drawable red = getResources().getDrawable(R.drawable.duia_zb_huang);
            Drawable yellow = getResources().getDrawable(R.drawable.duiaz_zb_hong);
            Drawable blue = getResources().getDrawable(R.drawable.duia_zb_lan);
            ...
            dHeight = red.getIntrinsicHeight()/3;
            dWidth = red.getIntrinsicWidth()/3;
...
//    }
    ...
    private AnimatorSet getEnterAnimtor(final View target) {
        ...
        ObjectAnimator alpha = ObjectAnimator.ofFloat(target, View.ALPHA, 0.3f,
                1f);
        double xs;
        int sj =(int) (Math.random()*9+1);
        if(sj>=1){
            xs = Float.parseFloat("0."+sj);
        }else{
            xs=0.1;
        }
    ...
    private PointF getPointF(int scale) {
        ...
        pointF.x = random.nextInt((mWidth - 0));// 减去100 是为了控制 x轴活动范围,看效果 随意~~
        // 再Y轴上 为了确保第二个点 在第一个点之上,我把Y分成了上下两半 这样动画效果好一些 也可以用其他方法
        pointF.y = random.nextInt((mHeight - 0)) / scale;
        return pointF;
    }
